...
async function computeInsight(exerciseId: number): Promise<SmartInsight | null> {
...
export function useSmartInsight(exerciseId: number): SmartInsight | null | undefined {
  // useLiveQuery는 rejection을 렌더 중 throw하므로, DB 에러를 여기서 삼켜야
  // 백그라운드 복귀 시 DB 연결 종료가 앱 전체를 죽이지 않음
  return useLiveQuery(
    () =>
      computeInsight(exerciseId).catch((e) => {
        console.error('[useSmartInsight]', e);
        return null;
      }),
    [exerciseId]
  );
